...
export const vaultwarden: GameTemplate = {
  ...
  resources: { memoryMb: 512, cpuPercent: 100, diskMb: 2048 },
  features: ["web"],
  // ROCKET_PORT makes Vaultwarden listen on the allocated host port (avoids 80/8080).
  ports: [{ name: "HTTP", protocol: "tcp", default: 8103, envVar: "ROCKET_PORT", primary: true }],
  variables: [
...
export const nextcloud: GameTemplate = {
  ...
  features: ["web"],
  // Host 8102 (80=panel, 8080=daemon taken); Apache listens on 80 inside.
  ports: [{ name: "HTTP", protocol: "tcp", default: 8102, containerPort: 80, primary: true }],
  variables: [
